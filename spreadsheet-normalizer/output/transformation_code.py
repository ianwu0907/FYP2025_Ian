...
def transform(df):
    ...
    try:
        ...
        if split_result.shape[1] >= 2:
            result['item_category_zh'] = split_result[0]
            result['item_gender_zh'] = split_result[1]
    ...
    try:
        ...
        if split_result.shape[1] >= 2:
            result['item_category_en'] = split_result[0]
            result['item_gender_en'] = split_result[1]
    ...
    # reported_to_police_zh: categorical, Based on metadata: unique_values=['不適用', '有', '沒有']
    try:
        mapping = {'不適用': 'N/A', '有': 'Yes', '沒有': 'No'}
        result['有否向警方舉報事件'] = result['有否向警方舉報事件'].map(mapping)
        print('Converted reported_to_police_zh to categorical')
    except Exception as e:
        print(f'Error converting reported_to_police_zh: {e}')

    # reported_to_police_en: categorical, Based on metadata: unique_values=['Yes', 'No']
    try:
        mapping = {'Yes': 'Yes', 'No': 'No'}
        result['Incident Being or Not Reported to Police'] = result['Incident Being or Not Reported to Police'].map(mapping)
        print('Converted reported_to_police_en to categorical')
    except Exception as e:
        print(f'Error converting reported_to_police_en: {e}')

    # category_zh: categorical, Based on metadata: unique_values=['虐待長者性質', '虐待長者性質及受虐長者性別', '施虐者與受虐長者關係', '受虐長者居住地區']
    try:
        mapping = {'虐待長者性質': 'Type of Elder Abuse', '虐待長者性質及受虐長者性別': 'Type of Elder Abuse and Sex of Elderly Person Being Abused', '施虐者與受虐長者關係': "Abuser's Relationship with Elderly Person Being Abused", '受虐長者居住地區': 'Residential District of Elderly Person Being Abused'}
        result['類別'] = result['類別'].map(mapping)
        print('Converted category_zh to categorical')
    except Exception as e:
        print(f'Error converting category_zh: {e}')

    # category_en: categorical, Based on metadata: unique_values=['Type of Elder Abuse', 'Type of Elder Abuse and Sex of Elderly Person Being Abused', "Abuser's Relationship with Elderly Person Being Abused", 'Residential District of Elderly Person Being Abused']
    try:
        mapping = {'Type of Elder Abuse': 'Type of Elder Abuse', 'Type of Elder Abuse and Sex of Elderly Person Being Abused': 'Type of Elder Abuse and Sex of Elderly Person Being Abused', "Abuser's Relationship with Elderly Person Being Abused": "Abuser's Relationship with Elderly Person Being Abused", 'Residential District of Elderly Person Being Abused': 'Residential District of Elderly Person Being Abused'}
        result['Category'] = result['Category'].map(mapping)
        print('Converted category_en to categorical')
    except Exception as e:
        print(f'Error converting category_en: {e}')

    # year: integer
    try:
        result['年份/Year'] = pd.to_numeric(result['年份/Year'], errors='coerce').astype('Int64')
    ...
    try:
        result['個案數字/No. of Cases'] = pd.to_numeric(result['個案數字/No. of Cases'], errors='coerce').astype('Int64')
    ...
    expected_columns = ['year', 'reported_to_police_zh', 'reported_to_police_en', 'category_zh', 'category_en', 'item_category_zh', 'item_gender_zh', 'item_category_en', 'item_gender_en', 'number_of_cases']
