...
Be thorough and specific. Output only the JSON."""
    ...
    def _create_sample_data_view(self, df: pd.DataFrame, structure_analysis: Dict[str, Any]) -> str:
        ...
        start_row_raw = data_region.get('start_row', 1)
        end_row_raw = data_region.get('end_row', len(df)-1)

        # Convert to int
        try:
            start_row = int(start_row_raw) if start_row_raw is not None else 1
        except (ValueError, TypeError):
            start_row = 1
        try:
            end_row = int(end_row_raw) if end_row_raw is not None else len(df)-1
        except (ValueError, TypeError):
            end_row = len(df)-1

        end_row = min(end_row, start_row + 15)
...
Generate the Python code now. Output ONLY code, no explanations or markdown."""
    ...
    def _validate_result(self,
        ...
        expected_count_raw = schema.get('expected_output', {}).get('row_count_estimate', 0)
        # Convert to int, handle string values from LLM
        try:
            expected_count = int(expected_count_raw) if expected_count_raw else 0
        except (ValueError, TypeError):
            expected_count = 0
