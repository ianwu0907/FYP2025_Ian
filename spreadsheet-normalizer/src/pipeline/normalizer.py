"""
Table Normalizer Pipeline - All-in-One
Includes: Empty row detection + Hyperlink detection + Processing pipeline
"""
...
class TableNormalizer:
    ...
    Features:
    1. Empty row detection (built-in)
    2. Hyperlink detection (built-in)
    3. Auto detection mode (try hyperlink first, fallback to empty row)
    """
    ...
    def normalize(self, input_file: str, output_file: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.enable_table_splitting:
                logger.info("\n" + "="*80)
                logger.info("STAGE 1: TABLE DETECTION")
                logger.info("="*80)
                
                split_tables = self._detect_tables_smart(input_file)
                ...
                if len(split_tables) > 1:
                    logger.info(f"Detected {len(split_tables)} tables using {split_tables[0]['detection_method']}")
                    return self._process_multiple_tables(
                        split_tables, pipeline_log, start_time, output_file
                    )
                else:
                    logger.info("Single table detected, continuing with normal pipeline")
            
            # Stage 1 (fallback): Encoding
            logger.info("\n" + "="*80)
            ...
            if self.save_intermediate:
                ...
                if 'encoded_text' in encoded_data_to_save:
                    encoded_data_to_save['encoded_text'] = encoded_data['encoded_text'][:1000]
                
                self._save_intermediate('encoded_data.json', encoded_data_to_save)
