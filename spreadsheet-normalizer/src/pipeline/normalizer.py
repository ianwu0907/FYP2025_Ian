...
class TableNormalizer:
    ...
    def __init__(self, config: Dict[str, Any], progress_callback: Optional[Callable] = None):
        ...
        Args:
            ...
            progress_callback: Optional callback function (stage: str, progress: int, message: str) -> None
        """
        self.config = config
    ...
    def normalize(self, input_file: str, output_file: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            # ================================================================
            # Stage 1: ENCODING (10-25%)
            # ================================================================
            logger.info("\n" + "=" * 80)
            logger.info("STAGE 1: SPREADSHEET ENCODING")
            logger.info("=" * 80)
            self._report_progress("encoding", 10, "Loading and encoding spreadsheet structure...")
            ...
            self._report_progress("encoding", 25, "Spreadsheet encoding completed")

            # ================================================================
            # Stage 2: STRUCTURE ANALYSIS (25-45%) (LLM Semantic Reasoning)
            # ================================================================
            logger.info("\n" + "=" * 80)
            logger.info("STAGE 2: STRUCTURE ANALYSIS (Semantic Reasoning)")
            logger.info("=" * 80)
            self._report_progress("analyzing", 30, "Analyzing table structure with LLM...")
            ...
            self._report_progress("analyzing", 45, "Structure analysis completed")

            # ================================================================
            # Stage 3: SCHEMA ESTIMATION (45-65%) (Tidy Data Principles)
            # ================================================================
            logger.info("\n" + "=" * 80)
            logger.info("STAGE 3: SCHEMA ESTIMATION (Tidy Data Principles)")
            logger.info("=" * 80)
            self._report_progress("estimating", 50, "Estimating normalized schema with LLM...")
            ...
            self._report_progress("estimating", 65, "Schema estimation completed")

            # ================================================================
            # Stage 4: TRANSFORMATION (65-90%) (Strategy → Code → Execute → Validate)
            # ================================================================
            logger.info("\n" + "=" * 80)
            logger.info("STAGE 4: TRANSFORMATION (Two-Stage Generation)")
            logger.info("=" * 80)
            self._report_progress("transforming", 70, "Generating transformation strategy...")
            ...
            self._report_progress("transforming", 90, "Transformation completed")

            # ================================================================
            # Stage 5: SAVE OUTPUT (90-100%)
            # ================================================================
            logger.info("\n" + "=" * 80)
            logger.info("STAGE 5: SAVING OUTPUT")
            logger.info("=" * 80)
            self._report_progress("saving", 95, "Saving normalized output...")

            if output_file is None:
                output_file = self.output_dir / 'normalized_output.csv'

            # Check if normalized_df is empty
            df_to_save = transformation_result['normalized_df']
            if df_to_save.empty:
                logger.warning("⚠️ Normalized DataFrame is empty! Falling back to original DataFrame.")
                logger.warning("This usually indicates that the transformation code filtered out all data.")
                logger.warning("Please review the intermediate files to debug the transformation logic.")
                df_to_save = encoded_data['dataframe']
                pipeline_log['fallback_to_original'] = True

            output_path = self._save_output(df_to_save, output_file)
            ...
            self._report_progress("completed", 100, "Normalization completed successfully!")
